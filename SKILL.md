...
- **Growth, metrics & experimentation:** McClure (AARRR); Verna (hybrid-PLG, PQA); Rachitsky (Lenny's); Amplitude (North Star); Google (HEART); Eyal (Hook); Ries (Lean Startup / Build-Measure-Learn); Kohavi/Tang/Xu (*Trustworthy Online Controlled Experiments* — SRM, Twyman's law, peeking); Raviv (experiment craft); L.E.K. (key value drivers).
...
| Metrics & North Star | `metrics` | North Star + input-metric tree, value-driver tree, guardrail design, agent metrics | `pro/metrics.md` |
| Strategy & Roadmap | `strategy` | Strategy Blocks & P2W, platform/product-family + multi-sided-platform strategy, IfM roadmapping, Aumayr Business Plan | `pro/strategy.md` |
